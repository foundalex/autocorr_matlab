...
MAX1 = round(VAL*ATAN_LUT_SCALE/4);
...
%%
% angle_rad_int_shift = (-angle_rad_int)/16;
pi_int = round(pi*ATAN_LUT_SCALE/4);
next_phase_correction = 0;

for i = 1:n

    if ((angle_rad_int)/ATAN_LUT_SCALE < 0)
        next_phase_correction = next_phase_correction - abs((angle_rad_int)/2^16/4);
...
%     indexiii(i) = index;
    dds_cos(i) = ii(abs(round(next_phase_correction)));
    dds_sin(i) = qq(abs(round(next_phase_correction)));
end
%         if (quadrant == 0)
%             rot_i(i) = sin1(i);
%             rot_q(i) = cos1(i);

...
load('test_signals\wlan_gen_20_MHz_4_MCS_3_packet_without_noise.mat');
rxWaveform = [waveStruct.waveform];
rxWaveform = rxWaveform(20000:end); % пропускаем преамбулу
a = real(rxWaveform);
plot(a)
rxWaveform_receive = rxWaveform;
...
for i = 1:2
    thres_idx = find(M(init_value:end) > threshold); % находим пики корреляции больше 0.75
    thres_idx_array(i) = thres_idx(1) + init_value - 1; % создаем массив с индексами корреляции
...
for i = 1:length(thres_idx_array)
    if (i == length(thres_idx_array))
        ...
        rxWaveform = rxWaveform_receive(thres_idx_array(i)-1:thres_idx_array(i+1));
    end
